...
- 🟢 **Repo-verified (2026-09-16):** The fixed bottom-right auth tray could compete with the composer and notifications, especially when a signed-in user's email widened the control. Auth actions now render in the sticky conversation header and wrap to their own row at mobile widths; desktop placement and frontend lint/build were verified.
- 🟢 **Repo-verified (2026-09-16):** Folder uploads already preserved `webkitRelativePath`, but the browser discarded every path deeper than one folder before upload. The picker now sends all supported files recursively; frontend lint/build and the four existing recursive ingestion tests pass.
